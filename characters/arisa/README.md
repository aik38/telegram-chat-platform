...
## 恋愛モードの3パターン（LOVE_A / LOVE_B / LOVE_C）
- 恋愛モードは LOVE_A / LOVE_B / LOVE_C の3パターンで距離感・主導権・言葉量を調整する。
- Love（romance）モードにのみ適用され、base/sexy には影響しない。
- ユーザーには明示しない。恋愛ボタン押下時にランダムで選ばれる。

### 管理者による固定コマンド
- 管理者のみが Telegram コマンドで固定できる（`ADMIN_USER_IDS`）。
- `/love_a` / `/love_b` / `/love_c` → 以降の恋愛モード会話で指定型を使用。
