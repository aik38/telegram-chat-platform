...
## Love Style Card（恋愛モード）
- Love Style Card は、人格を増やすのではなく、会話の作法（距離感・テンポ・甘え方・質問頻度等）を切り替える仕組み。
- Love（romance）モードにのみ適用され、base/sexy には影響しない。
- UI変更なしで「雰囲気が変わる」ことを狙い、ユーザーは Style を選択できない。
...
### 管理者による切り替え方法
- 管理者のみが Telegram コマンドで操作できる（`ADMIN_USER_IDS`）。
- `/love_style` → 現在のカード番号 + 名称を表示。
- `/love_style 1`〜`/love_style 8` → Style Card を即時切り替え。
- 変更は全ユーザーにグローバル適用される。
- 設定はランタイムのグローバル値として扱う（永続化はしない）。

### 一般ユーザーの挙動
- 一般ユーザーには UI や操作導線は表示されない。
- 返答の雰囲気や距離感の変化としてのみ体験される。
