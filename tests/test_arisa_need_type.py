...
def test_classify_need_by_keywords(monkeypatch):
    ...
    assert bot_main.classify_need("あんしんしたい") == "calm"
    assert bot_main.classify_need("刺激がほしい") == "tease"
    assert bot_main.classify_need("しげきがほしい") == "tease"
    assert bot_main.classify_need("整理したい") == "clarify"
    assert bot_main.classify_need("せいりしたい") == "clarify"
    assert bot_main.classify_need("安心と刺激、どっちも") == "calm"
    assert bot_main.classify_need("今日は疲れた") is None


def test_resolve_need_type_defaults_to_unknown(monkeypatch):
    bot_main = import_bot_main(monkeypatch)

    assert bot_main.resolve_arisa_need_type(None, "今日は疲れた") == "unknown"


def test_get_user_calling(monkeypatch):
    bot_main = import_bot_main(monkeypatch)

    assert bot_main.get_user_calling(paid=True, known_name="Arisa") == "Arisaさん"
    assert bot_main.get_user_calling(paid=True, known_name=None) == "あなた"
    assert bot_main.get_user_calling(paid=False, known_name="Arisa") == "あなた"
    assert bot_main.get_user_calling(paid=False, known_name=None) == "あなた"
