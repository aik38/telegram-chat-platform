...
echo Running: %PS% -NoProfile -ExecutionPolicy Bypass -File "%REPO%scripts\run_arisa.ps1" -DotenvFile "%DOTENV_PATH%"
%PS% -NoProfile -ExecutionPolicy Bypass -Command "& { $ErrorActionPreference='Stop'; & '%REPO%scripts\run_arisa.ps1' -DotenvFile '%DOTENV_PATH%' 2>&1 | Tee-Object -FilePath '%LOG_FILE%' -Append; exit $LASTEXITCODE }"
if errorlevel 1 (
