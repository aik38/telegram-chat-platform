...
このドキュメントを **唯一のWBS** とし、進捗スナップショットは `docs/WBS_PROGRESS.md` に記録します（最終更新: 2025-12-20 UTC）。見た目よりも「落ちない」「迷子にならない」を最優先で維持します。
...
## 🚫 Not doing（理由つき）
- T3-07: 相談モード仕様確定（回数制限と文体）は現行仕様を維持する方針が固まっており、本サイクルでは再設計しない。
- T4-07: 料金最適化（モデル選定、max_tokens、キャッシュ、リトライ）は gpt-4o-mini＋環境変数しきい値の現行運用を維持する。コスト方針は既定のため追加実装なし。
...
1. [~] T4-02: タロット用プロンプトテンプレ（安定した型）。完了条件: core/prompts.py のテンプレが安定し、LLM 出力の型揺れを抑制するユニットテストが追加される。
2. [ ] T4-08: API分離の要否（Bot直呼び vs FastAPI経由）。完了条件: FastAPI 経由の実装・測定または Bot 直呼び継続の根拠が文書化される。
3. [ ] T5-09: 不正対策（複垢/同端末/再インストール）。完了条件: 端末・アカウントの異常検知ルールを決め、最低限のブロック/警告フローを docs に追記。
4. [ ] T5-10: コスト監視（ユーザー別トークン/日次上限）。完了条件: 日次コスト集計または OpenAI usage の簡易ダンプ運用を決め、ルールを README/docs に記載。
5. [ ] T6-03: sessions（会話セッション）永続化。完了条件: 最低限のセッションテーブルを追加し、開始/完了/タイムアウトを保存できる。
6. [ ] T6-04: messages（入出力ログ：要点だけ）。完了条件: ユーザー入力と要約出力を保存し、個人情報をマスクする仕組みを決める。
7. [ ] T6-07: app_events（イベント計測：start/reading_done/buy_click…）。完了条件: 主要イベントを event テーブルに保存し、失敗時も bot が落ちない。
8. [ ] T7-04: 監視（ヘルスチェック/死活監視/通知）。完了条件: Uvicorn/Bot のヘルスチェックエンドポイントか手順が決まり、監視運用の連絡先が定義される。
9. [ ] T8A-04: 相談モードの価値定義（短文・共感・行動提案）。完了条件: 現行の相談モード文体を守りつつ、価値の説明と行動提案例が /help か README に整理される。
10. [ ] T8A-05: “購入”の心理導線（無料→体験→不足→解決）。完了条件: /start, /buy, /status での案内文とボタン導線が一貫し、無料枠からの誘導が1画面で確認できる。

## 🚀 Launch (public + marketing) - 48h checklist

- [ ] T-48h: `tools/sync.ps1` を実行し `pytest` が green（ログ保存）。
- [ ] T-48h: `.env` 本番値（BOT_TOKEN / OPENAI_API_KEY / 管理者ID / PAYWALL）を再確認。
- [ ] T-24h: 決済スモーク（/buy→購入→/status反映、/buy連打で dedup、stale callback が安全に案内）。
- [ ] T-24h: SQLite backup/restore 手順を読み上げテストし、復旧イメージを共有。
- [ ] T-12h: /start と /help の文面を最終化（質問例・料金・次アクションが1画面で伝わる）。
...
## フェーズ8-10: MVPローンチ最短経路（優先順3-7件）
1. [ ] T8A-04: 相談モードの価値定義（短文・共感・行動提案）― 現行文体を守りつつ、価値説明と提案例を /help か README に明記。
2. [ ] T8A-05: “購入”の心理導線（無料→体験→不足→解決）― /start, /buy, /status の文面とボタン導線を一貫させ、無料枠からの案内が1画面で確認できること。
3. [ ] T8A-06: メッセージ文体の統一（丁寧でやさしい敬語）― 主要メッセージの文体ガイドを決め、bot/texts/ja.py と README に反映。
4. [ ] T9-01: キャラ設定シート（口調・価値観・NG・得意領域）― Bot の人格と避ける表現を1枚に集約。
5. [ ] T10-01: 価値提案を1文で固定（誰の何をどう解決するか）― 外部発信や /start で使える定型文を決める。
6. [ ] T10-05: KPI設計（開始率/完了率/課金率/継続率/粗利）― Beta で追う指標と取得方法を明文化。
